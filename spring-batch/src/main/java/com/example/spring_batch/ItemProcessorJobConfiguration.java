...
public class ItemProcessorJobConfiguration {
    ...
    public Step step(
        ...
        final List<ItemProcessor<User, User>> processorList = Arrays.asList(processor1(),processor2(), processor3());

        return new StepBuilder("step", jobRepository)
                .<User, User>chunk(2, platformTransactionManager)
                .reader(jpaCursorItemReader)
//                .processor(cumtomProcessor())
                .processor(new CompositeItemProcessor<>(processorList))
                .writer(System.out::println)
                .build();
    }

    private static ItemProcessor<User, String> cumtomProcessor() {
        return user -> {
            if (user.getName().equals("홍")) return null;

            return "%s의 나이는 %s입니다. 사는 곳은 %s, 전화 번호는 %s 입니다.".formatted(
